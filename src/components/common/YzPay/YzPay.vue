                ...
                <div class="yz-pay-header">
                    ...
                    <div class="yz-pay-header-right">
                        <div class="yz-pay-header-amount">
                            ￥<span>{{ pageDetail?.payAmount }}</span>元
                        </div>
                    </div>
                </div>
            </div>
...
import { orderPay, placeOrderMain, getPayGoodsDetailById } from '@/api/pay';
...
const onPayBondClick = async () => {
    ...
    if (payTypeData.value.payType == 'alipay' || payTypeData.value.payType == 'wechat') {
        ...
        const payRes = await orderPay(props.goodsId, orderNo.value, payTypeData.value.payType, payTypeData.value.balanceType)
        if (payRes.code == 200) {
            console.log('支付成功');
            //本地账户支付成功待处理
            emtis('paySuccess', {
...
const init = async () => {
    try {
        const res = await placeOrderMain(props.goodsId, props.businessId,props.tranNum);
        if (res.code != 200) {
    ...
    if (isArray(props.businessId)) {
        pageDetail.value.payAmount = Number(pageDetail.value.payAmount) * props.businessId.length;
    }
};
