...
<template>
  <div class="multi-img-upload" :class="{ 'single-center': localImgTypes.length === 1 }">
    <div v-for="item in localImgTypes" :key="item.name" class="img-upload-item">
      ...
      <el-upload :disabled="preview === true" class="avatar-uploader" action="" :http-request="async (options: UploadRequestOptions) => {
        ...
        <YzImage class="image-slot avatar" v-if="!isEmptyVal(item.url)" :src="item.url" v-model:oss-url="item.signedUrl" :is-oss="true"></YzImage>
