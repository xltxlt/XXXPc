...
import { documentService } from "@/api/index.ts";
...
const pageConfig: TempListPageConfig = {
  url: documentService.apiDocumentPagelistPost.bind(documentService),
  optionUrl: documentService.apiDocumentPageoptionGet.bind(documentService),
  pageFun: {
    add: (data: any) => {
      return {
        title: "新增",
    ...
    batchDel: async (data: any[]) => {
      ...
      const res = await documentService.apiDocumentBatchlogicdeletePost(ids);
    ...
    del: async (data: any) => {
      ...
      const res = await documentService.apiDocumentLogicdeleteIdPost(data.id);
      handleSumbitResBox(res, '删除成功', () => {
        pageInfoRef.value?.onRefresh();
      });
    },
