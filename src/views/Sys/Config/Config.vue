...
const pageConfig: TempListPageConfig = {
  url: configService.apiSysConfigPagelistPost.bind(configService),
  optionUrl: configService.apiSysConfigPageoptionGet.bind(configService),
  pageFun: {
    add: (data: any) => {
      return {
        title: "新增",
    ...
    batchDel: async (data: any[]) => {
      ...
      const res = await configService.apiSysConfigBatchlogicdeletePost(ids);
    ...
    del: async (data: any) => {
      ...
      const res = await configService.apiSysConfigLogicdeleteIdPost(data.id);
      handleSumbitResBox(res, '删除成功', () => {
        pageInfoRef.value?.onRefresh();
      });
    },
