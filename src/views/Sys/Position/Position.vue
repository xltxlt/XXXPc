...
const pageConfig: TempListPageConfig = {
  url: positionService.apiSysPositionPagelistPost.bind(positionService),
  optionUrl: positionService.apiSysPositionPageoptionGet.bind(positionService),
  pageFun: {
    add: (data: any) => {
      return {
        title: "新增",
    ...
    batchDel: async (data: any[]) => {
      ...
      const res = await positionService.apiSysPositionBatchlogicdeletePost(ids);
    ...
    del: async (data: any) => {
      ...
      const res = await positionService.apiSysPositionLogicdeleteIdPost(data.id);
      handleSumbitResBox(res, '删除成功', () => {
        pageInfoRef.value?.onRefresh();
      });
    },
